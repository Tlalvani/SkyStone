...
/*
     ...
     Drive(RampPower(3615, .5,.2),RampPower(3615, .5,.2));
        while (robot.LB.isBusy() & robot.RF.isBusy() & robot.LF.isBusy() & robot.RB.isBusy() & opModeIsActive()) {
...
*/
     ...
     robot.AutoArmRotate.setPosition(robot.servorotateback);
        ...
        StoneMove(skystone,-3215,-3015,-2815);
       Drive(RampPower(-3215, .5,.2),RampPower(-3215, .5,.2));
        while (robot.LB.isBusy() & robot.RF.isBusy() & robot.LF.isBusy() & robot.RB.isBusy() & opModeIsActive()) {
            if (robot.RB.getCurrentPosition() > (robot.RB.getTargetPosition() + 2600)) {
                robot.AutoArmRotate.setPosition(robot.servorotateback);
            } else {
